...
function getServiceListConfig(widgetTag: string): ServiceListConfig | null {
  ...
  return {
    ...
    services: serviceElList.map(el => el.textContent?.trim()).filter(s => s !== undefined),
  }
}

const ServiceList = ({ widgetTag, onSend }: ServiceListProps) => {
  const config = getServiceListConfig(widgetTag)

  if (!config?.services || config.services.length === 0) return <></>

  return <div className='mb-[10px] mt-[13px] rounded-[20.8px] rounded-tl-none border border-green-50 bg-[rgba(235,235,235,0.4)] px-[16px] py-[28px]'>
    ...
    <ul className='mt-6 flex flex-col gap-2'>
      {
        config.services.map(service => (
          <li>
            <div key={service} className='flex h-10 w-full items-center justify-between rounded-3xl bg-white p-0.5 px-4' onClick={() => onSend?.(service)}>
...
export function isServiceList(widgetTagStr?: string) {
  if (!widgetTagStr) return false
  return widgetTagStr.startsWith('<service-list') && widgetTagStr.endsWith('</service-list>')
}
