...
function getProductRecommandConfig(node: any): ProductRecommandConfig | null {
  ...
  const productsRawInfosStr = node.children.filter((el: any) => el.tagName && el.tagName.toLowerCase() === 'product-raw-info')
  ...
  try {
    ...
    const recommandProductIdsRaw = node.children.filter((el: any) => el.tagName && el.tagName?.toLowerCase() === 'recommand-product-ids')
      .map((riEl: any) => riEl.children.find((el: any) => el.type === 'text' && el.value.trim().length > 0))?.[0]?.value
