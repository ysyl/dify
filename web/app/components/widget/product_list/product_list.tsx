...
const TITLE_ICON = () => <svg width="18" height="18" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function getProductListConfig(widgetTag: string): ProductListConfig | null {
  ...
  if (!productsRawInfosStr) {
    ...
    const productConfigList: ProductInfo[] = productElList.map(el => ({
      group: el?.querySelector('group')?.attributes.getNamedItem('value')?.value || '',
...
export function isProductList(widgetTagStr?: string) {
  ...
  return widgetTagStr.startsWith('<product-list') && widgetTagStr.endsWith('</product-list>')
}
